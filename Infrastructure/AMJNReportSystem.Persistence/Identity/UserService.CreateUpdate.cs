...
namespace AMJNReportSystem.Persistence.Identity
{
    public partial class UserService
    {
        ...
        public async Task<string> CreateAsync(CreateUserRequest request, string origin)
        {
            ...
            await _userManager.AddToRoleAsync(user, GXRoles.Nazim);
        ...
        {
            const int length = 8; // Desired length of the membership number

            string membershipNumber = string.Empty;
            for (int i = 0; i < length; i++)
            {
                membershipNumber += random.Next(10); // Generate a random digit (0-9)
            }
